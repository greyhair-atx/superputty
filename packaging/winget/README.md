...
`-RetryCleanedRegistrationFailure` supports the reviewed HKCU-probe failure only when the successful installation, exact diagnostic error, successful cleanup, and preservation records match. Both PowerShell 5.1 and 7 retry-guard checks passed. WinGet lifecycle testing is still incomplete.
