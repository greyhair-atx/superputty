...
if(Test-Path "$stage/results/controller-started.txt"){
    ...
    $cleanedRegistrationOnly=$false
    if($RetryCleanedRegistrationFailure -and $ops.Count -eq 4){
        $cleanupLog=Get-Content "$stage/results/user/cleanup-msi.log" -Raw
        $errorLog=Get-Content "$stage/results/user-error.log" -Raw
        $cleanedRegistrationOnly=($ops[0].name -eq 'version' -and $ops[0].exitCode -eq 0 -and $ops[1].name -eq 'effective-settings' -and $ops[1].exitCode -eq 0 -and $ops[2].name -eq 'validate' -and $ops[2].exitCode -eq 0 -and $ops[3].name -eq 'install' -and $ops[3].exitCode -eq 0 -and $cleanupLog.Contains('MainEngineThread is returning 0') -and $errorLog.Contains('PathNotFound') -and $errorLog.Contains('7093D12F-0585-4E0C-A625-D2BB3867C54F'))
    }
    if($previous.success -or -not $previous.hostFilesPreserved -or -not $previous.localManifestSettingRestored -or ($previous.PSObject.Properties.Name -contains 'localManifestPolicyRestored' -and -not $previous.localManifestPolicyRestored) -or -not $scopeResult.cleanupSucceeded -or -not $scopeResult.settingsPreserved -or -not (($RetryFailedBootstrap -and $bootstrapOnly) -or $cleanedUninstallOnly -or $cleanedRegistrationOnly) -or (Test-Path "$stage/results/machine")){throw 'Retry requires the reviewed failure with successful cleanup and preservation checks.'}
