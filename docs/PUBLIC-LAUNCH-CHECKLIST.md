...
- [x] Submit the schema 1.12.0 manifests to `microsoft/winget-pkgs`: [PR #434081](https://github.com/microsoft/winget-pkgs/pull/434081). Local validation and the complete current-user WinGet lifecycle passed, including silent installation/uninstallation and settings preservation. Both installer scopes passed the earlier MSI lifecycle and upgrade matrix.
- [ ] Monitor the WinGet submission checks and moderator review until catalog publication. An explicit all-users installation through WinGet remains useful follow-up evidence. See [WinGet staging](../packaging/winget/README.md).
