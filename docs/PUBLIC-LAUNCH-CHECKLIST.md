...
- [ ] Test installation through WinGet for both scopes, then submit to `microsoft/winget-pkgs`. The maintainer has authorized this work; schema 1.12.0 validation passed and the submission branch is prepared. Two VM attempts stopped before MSI execution: first at App Installer registration, then at account-specific local-manifest permission. Preservation/restoration checks passed. The corrected runner registers App Installer and temporarily enables the documented machine policy; it awaits an elevated retry. No successful WinGet install test or pull request is recorded yet. See [WinGet staging](../packaging/winget/README.md).
