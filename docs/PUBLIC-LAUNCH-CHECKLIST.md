...
- [ ] Test installation through WinGet for both scopes, then submit to `microsoft/winget-pkgs`. Schema 1.12.0 validation and the complete current-user WinGet lifecycle passed, including silent installation/uninstallation and settings preservation. The submission branch is prepared. The all-users WinGet lifecycle and pull request remain pending. See [WinGet staging](../packaging/winget/README.md).
